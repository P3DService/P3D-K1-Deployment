...
## K1 Max: задний вентилятор отображается включённым, но не крутится

Если экран K1 Max показывает работу заднего/chamber вентилятора, но физически он стоит, а ручное переключение `0% -> 100%` сразу его запускает:

- [K1 Max: задний вентилятор не стартует на пониженной скорости](K1_MAX_REAR_FAN_START.md)

P3D workaround добавляет 500 мс стартового импульса 100% только при запуске `fan1` из 0 на пониженном PWM.
...
Если `stepper_x` / `stepper_y` перегревается во время Bed Mesh, Input Shaper или теста резонансов при холодном хотэнде, см. проверенное на K1 Max решение:

- [Перегрев TMC2209 во время холодной калибровки](TMC2209_OVERHEAT.md)

В подтверждённом случае вентилятор платы был объединён с вентилятором хотэнда и выключался ниже порога температуры экструдера.


## Bed Mesh идёт много минут / большой уклон спереди назад

Если сетка 6×6 визуально прошла обычные 36 точек, а затем калибровка продолжается ещё несколько минут, проверьте `klippy.log` на `RE_PROBE_POINT`. Большой плавный уклон может запускать повторные проверки PRTouch — это не обязательно зависание интерфейса.

Подробный field-validated кейс K1 Max:

- [K1 Max: большой уклон Bed Mesh, повторные замеры и синхронизация Z](BED_MESH_Z_SYNC.md)
